...
echo -e "  • 권장:         ${B}npm run ud -- 1.6.0-beta.9 \"chore(release): v1.6.0-beta.9\"${R}"
echo -e "  • 수동:         ${B}npm run build && npm run deploy${R}"
echo -e ""

echo -e "${B}${C4}[5] 버전 올리는 규칙${R}"
echo -e "  • 현재 단계: ${B}1.6.0-beta.9${R}"
...
echo -e "     - do-kkae-bi-onboarding-v1.6.0-beta.9"
echo -e "     - do-kkae-bi-onboarding-runtime-v2"
echo -e "  3) RTDB inventory/logs 경로 데이터 수신 여부"
echo -e ""
echo -e "${B}${C4}[10] 1.6.0-beta.9 릴리즈 게이트 점검 (가이드/기록페이지)${R}"
echo -e "  • Home↔Logs 이동 / 새로고침 / 뒤로가기 후 checklist·coach·step 상태가 유지되는지 확인"
echo -e "  • completed 이후 skip 버튼이 보이지 않고, 수동 재오픈 시 복습 모드로만 열리는지 확인"
echo -e "  • Logs guide dock / toolbar / toast / popup이 모바일에서 서로 겹치지 않는지 확인"
echo -e "  • 일괄 추가 전 30분 백업 생성, 복구, TTL 만료가 깨지지 않는지 확인"
echo -e ""