...
echo -e "  • 권장:         ${B}npm run ud -- 1.6.0-beta.6 \"chore(release): v1.6.0-beta.6\"${R}"
echo -e "  • 수동:         ${B}npm run build && npm run deploy${R}"
echo -e ""

echo -e "${B}${C4}[5] 버전 올리는 규칙${R}"
echo -e "  • 현재 단계: ${B}1.6.0-beta.6${R}"
...
echo -e "     - do-kkae-bi-onboarding-v1.6.0-beta.6"
echo -e "     - do-kkae-bi-onboarding-runtime-v1"
echo -e "  3) RTDB inventory/logs 경로 데이터 수신 여부"
echo -e ""
echo -e "${B}${C4}[10] 1.6.0-beta.6 핫픽스 점검 (가이드/기록페이지)${R}"
echo -e "  • Home/Logs 가이드 코치는 guide-floating-dock 내부에서만 렌더되는지 확인"
echo -e "  • step 1 -> step 2 -> /logs 진입 시 코치가 1회만 노출되는지 확인"
echo -e "  • Logs에서 필터 토글/내보내기 완료 후 Home 복귀 시 step 3으로 이어지는지 확인"
echo -e "  • 카드 확대보기(장소/전체) hover 시 pointer + focus-visible 피드백 확인"
echo -e ""